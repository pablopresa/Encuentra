...
public class _EncuentraAltaFileRecepcion extends Action 
{
			...
			throws Exception 
	{
		...
		if(proveedores==null) 
		{
			proveedores=Logica.darProveedoresParaOrdenes(true);
				            ...
				            while (rowIterator.hasNext()) 
				            {
				                ...
				                try
				                {
				                	if(pasada>0)
				                	{
					                    ...
					                    Cell artCel = row.getCell(1);
					                    Cell unidadesCel = row.getCell(2);            
					                   
					                    try
					                    {
					                    	orden = Integer.parseInt(ordenCel.getStringCellValue());
					                    }
					                    catch(Exception e)
					                    {
					                    	orden = (int) ordenCel.getNumericCellValue();
					                    }	
					                    ...
					                    try
					                    {
					                    	unidades = Integer.parseInt(unidadesCel.getStringCellValue());
					                    }
					                    catch(Exception e)
					                    {
					                    	unidades = (int) unidadesCel.getNumericCellValue();
					                    }		
		
					                   DataIDDescripcion linea = new DataIDDescripcion(unidades,orden,articulo);
					                   listaSort.add(linea);
