...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
	...
	<c:set var = "colapsar" scope = "request" value = "yes"/>
        ...
        function resize(idDiv)
        {
        	var text = document.getElementById(idDiv).innerHTML
        ...
        function corregir() 
		{
			...
			{
				$.ajax({
                           url: "<%=basePath%>CorregirCantVPicking.do",
                           type: "post",
                           dataType: "html",
                           data: { 
			...
			else
			{
            	...
            	<div class="row">
                	<div class="col-md-12">
                        ...
                        <h6>Si hay articulos sin código de barra los debe asociar.</h6>
                    </div>
                </div>              
                 <!-- /. ROW  -->
                <hr />
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                            	<table class="table table-striped table-bordered table-hover" id="encuentra-default">
                                    <thead class="thead-dark">
                                        <tr>
                                            <th class="text-center" style="padding: 0px;"> Usuario</th>
                                            <th class="text-center" style="padding: 0px;"> Pedido</th>
                                            <th class="text-center" style="padding: 0px;"> Articulo</th>
                                            <th class="text-center" style="padding: 0px;"> Descripción</th>
                                           <!--  <th class="text-center"> Stock</th>
                                            <th class="text-center"> Stock <br/> SAP</th> -->
											<th class="text-center" style="padding: 0px;"> Destino</th>
											<th class="text-center" style="padding: 0px;"> Solicitadas<br>(${totSol})</th>
											<th class="text-center" style="padding: 0px;"> Encontradas<br>(${totEnc})</th>
											<th class="text-center" style="padding: 0px;"> Verificadas<br>(${totSol})</th>
										</tr>
                                    </thead>
                                    <tbody id="myBody">
                                    	...
                                    	<c:forEach var="p" items="${pickings}">
											...
											<tr>
												...
												<td class="text-center" id="desc-${p.articulo}-${p.destino.id}-${p.idPedido}-${p.solicitud}"><c:out value="${p.descripcion}"></c:out></td>
												...
												<script type="text/javascript">
													resize("desc-${p.articulo}-${p.destino.id}-${p.idPedido}-${p.solicitud}")
												</script>
										</c:forEach>
									</tbody>
                            ...
                            <div class="text-center col-sm-12 col-lg-12"> 
								<div class="flex-container">
									...
									<a href="#" onclick="Transferir(),this.addEventListener('click', clickStopper, false);return false;"  class="btn btn-danger">Grabar<br/> Transf. de Stock</a>									
								</div>
							</div>
