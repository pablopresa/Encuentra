...
<body onload=" document.getElementById('clonarBultoQty').focus();">
	<c:if test="${uLogeado!=null}">
		<div class="rounded">
			...
			<div class="row">
				<div class="col-100">
					<form method="post"action="<%=basePath%>/selMovStockRecep.do">
						<table>
							<tr>
								<td colspan="3"><h2> Recepcionando documentos en deposito <strong style="color: red"> <c:out value="${uLogeado.deposito}"></c:out> </strong></h2></td>
							</tr>
							<tr>
								<td colspan="3"><a href="<%=basePath%>/v3/handHeld/MobRecepcionListaDocs.jsp"><div class="button">Elegir Documentos</div></a></td>
							</tr>
	                    ...
	                    <c:forEach var="d" items="${remitosIn}">
							<tr>						
								...
								<a href="#" onClick="if(confirm('Seguro que desea este Doc de la lista?'))
															{
																window.location='<%=basePath%>delMovStockRecep.do?nroDoc=${d.idOrigen}_${d.numeroDoc}'
			...
			<div class="row">
					<a href="<%=basePath%>/darBarrasControl.do"> <input class="col-100 button" type="submit" value="Verificarlos"/></a>
					<!--   <a href="<%=basePath%>DarDocumentosRecepcionables.do?D=${uLogeado.deposito}"> <input class="col-100 button button-warning" type="button" value="Confirmar sin verificar"/></a>-->
