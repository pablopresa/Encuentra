...
import beans.JPEGImageFileFilter;
...
public class NaiveSimilarityFinder extends JFrame {
	...
	private static final String basePath = "C:\\Users\\gonza\\Downloads\\imagenes\\";
	...
	public NaiveSimilarityFinder(File reference) throws IOException {
		...
		for (int o = 0; o < others.length; o++) {
			...
			JLabel ldist = new JLabel(
					"<html>" + others[o].getName() + "<br>" + String.format("% 13.3f", distances[o]) + "</html>");
	...
	private Color[][] calcSignature(RenderedImage i) 
	{
		...
		BufferedImage image = convertRenderedImage(i);
		try
		{
			BufferedImage crude = createAndWrite(image, Color.white, false, 60, "motorcycle-02.png");
	
			
			// construct the buffered image
			BufferedImage bImage    = new BufferedImage(crude.getWidth(null), crude.getHeight(null), BufferedImage.TYPE_INT_RGB);
			
			//obtain it's graphics
			Graphics2D bImageGraphics = bImage.createGraphics();
			
			//draw the Image (image) into the BufferedImage (bImage)
			bImageGraphics.drawImage(crude, null, null);
		...
		float[] prop = new float[] { 1f / 10f,2f / 10f, 3f / 10f,4f / 10f, 5f / 10f,6f / 10f, 7f / 10f,8f / 10f, 9f / 10f,10f / 10f };
		for (int x = 0; x < 9; x++)
			for (int y = 0; y < 9; y++)
	...
	{
		BufferedImage image = convertRenderedImage(other);
		try
		{
			BufferedImage crude = createAndWrite(image, Color.white, false, 60, "motorcycle-02.png");
