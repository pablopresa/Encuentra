		...
		if (noNco == null) {
			...
			if (esValido) {
				...
				if (voy.getSolicitada() == unidad) {
					...
					/*
					 * esto quiere decir que hay mas unidad ya está sumada
					 * 
				...
				uLog.registrarEvento(idPicking, 0, 107, 0, true, session.getId(),
						"El código del articulo no es valido");
				session.setAttribute("menError", "El código del articulo no es valido");
				return mapping.findForward("ok");
			}
		} else {
			/*********************************************
