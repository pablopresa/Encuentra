...
public class SincronizaEncuentraCYBE 
{
	...
	{	
		...
		try
		{
			...
			Fecha fec = new Fecha(fechaMYSQL);
			fecha = fec.darFechaToSQLSinMinutos();
		}
		catch (Exception e) 
		{
			Fecha fec = new Fecha();
		...
		try 
		{
			...
			Logica.persistir("update fechasincronizacionart set fechahora = CURRENT_TIMESTAMP()");
		...
		consultasIdDesc.add("select id, Descripcion from encuentra_maestros  WITH (NOLOCK) where tipo = 'seccion';");
		consultasIdDesc.add("select id, Descripcion from encuentra_maestros WITH (NOLOCK) where tipo = 'categoria';");
		consultasIdDesc.add("select id, Descripcion from encuentra_maestros WITH (NOLOCK) where tipo = 'marca';");
		consultasIdDesc.add("select id, Descripcion from encuentra_maestros WITH (NOLOCK) where tipo = 'clase';");
		consultasIdDesc.add("select id, Descripcion from encuentra_maestros WITH (NOLOCK) where tipo = 'genero';");
		consultasIdDesc.add("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; select id, Descripcion from encuentra_maestros WITH (NOLOCK) where tipo = 'color';");
		consultasIdDesc.add("select id, Descripcion from encuentra_maestros WITH (NOLOCK) where tipo = 'artdescripcion';");
		consultasIdDesc.add("select id, Descripcion from encuentra_maestros WITH (NOLOCK) where tipo = 'barra' union all select distinct IdArticulo id, IdArticulo Descripcion from encuentra_articulos;");
		consultasIdDesc.add("select id, Descripcion from encuentra_maestros WITH (NOLOCK) where tipo = 'temporada';");
		...
		try
		{
			...
			System.out.println("leyendo secciones" );
			...
			System.out.println("leyendo categorias" );
			...
			System.out.println("leyendo marcas" );
			...
			System.out.println("leyendo clases" );
			...
			System.out.println("leyendo generos" );
			...
			System.out.println("leyendo colores" );
			...
			try
			{
				...
				List<ArticuloForus> articulosForus = MSSQL.darArticuloVs("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT [IdArticulo],[Descripcion],[talle],[color],[IdTemporada],[Coleccion],[IdProveedor],[IdMarca],[IdClase],[IdCategoria] FROM encuentra_articulos WITH (NOLOCK)",idEmpresa);
				...
				{
					cons.append("('"+d.getIdArticulo()+"', '"+d.getDescripcion()+"', '"+d.getTalle()+"', '"+d.getColor()+"', "+d.getIdTemporada()+", "+d.getColeccion()+","+d.getIdProveedor()+","+d.getIdMarca()+","+d.getIdClase()+", "+d.getIdCategoria()+","+idEmpresa+"), ");
					if(counter==3000)
					{
