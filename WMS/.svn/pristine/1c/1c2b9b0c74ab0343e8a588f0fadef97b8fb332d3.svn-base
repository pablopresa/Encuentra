								...
								if(d.getId()==articulo.getSucursal())
								{
									...
									if(pos<=humbral && cantidadCentral>0)
									{
										...
										if(cantidadEnviar>0 && cantidadEnviar<=cantidadCentral)
										{
											...
											String justificacion = "Stock en Central:" +articulo.getStockCentral()+"<br/> Stock en el Local "+articulo.getStockLocal()+"<br/> Max en el local "+articulo.getMaxLocal()+" <br/> Vendió "+articulo.getVenta()+" y tiene la posición "+pos+" en el ranking";
				...
				else if (art.getCantidad()< art.getCantidadCentral())
				{
					...
					for (ArticuloReposicion articulo : art.getArticulosVendidos()) 
					{
						...
						if(cantidadEnviar>0 && cantidadEnviar<=cantidadCentral)
						{
							...
							String justificacion = "Stock en Central:" +articulo.getStockCentral()+"<br/> Stock en el Local "+articulo.getStockLocal()+"<br/> Max en el local "+articulo.getMaxLocal()+" <br/> Vendió "+articulo.getVenta()+"";
					...
					for (DepositoDimension d : art.getArticulosVendidos().get(0).getDepositos())// recorro los depositos el orden es para ese articulo 
					{
						for (ArticuloReposicion articulo : art.getArticulosVendidos()) 
						{
							...
							if(d.getId()==articulo.getSucursal())
							{
								...
								if(pos<=humbral && cantidadCentral>0)
								{
									String justificacion = "Stock en Central:" +articulo.getStockCentral()+"<br/> Stock en el Local "+articulo.getStockLocal()+"<br/> Vendió "+articulo.getVenta()+" y tiene la posición "+pos+" en el ranking";
