			...
			<div class="row">
				<div class="col-100">
					...
					<table style=" border-collapse: collapse; border-spacing: 10px; ">
						...
						<c:forEach var="r" items="${recepcionablesS}">
							 <c:forEach var="d" items="${r.documentos}">
							 	<c:forEach var="a" items="${d.lineas}">
							 		<tr>
							 		<td>${d.numeroDoc} </td>
							 		<td>${a.idArticulo} </td>
							 		<td>${a.cantidad} </td>
							 		<td id="tdVerificada${d.numeroDoc}_${a.idArticulo}">${a.cantidadVerificada}</td>
							 		...
							 		<c:if test="${a.cantidad!=a.cantidadVerificada}">
							 			<td id="tdOk${d.numeroDoc}_${a.idArticulo}">&nbsp;</td>
							 		</c:if>
							 		<tr>
							 	</c:forEach>
							</c:forEach>
			...
			if (cod != "") 
			{
				...
				if(indice!=-1)
				{
					...
					if(indice2!=-1)
					{
						...
						var cantVer = document.getElementById("tdVerificada"+movimientoStock+"_"+articuloEnBarra).innerHTML;
						...
						xhttp.onreadystatechange = function() {
						  ...
						  xhttp.open("GET", "<%=basePath%>setCantArtMovStockRecep.do?doc="+movimientoStock+"&articulo="+articuloEnBarra, true);
						  xhttp.send();
						
						document.getElementById("tdVerificada"+movimientoStock+"_"+articuloEnBarra).innerHTML = parseInt(cantVer)+1;
						
						count++;
						
						if(cantidadMovimiento==0)
						{
							 document.getElementById("tdOk"+movimientoStock+"_"+articuloEnBarra).innerHTML = "<img src='<%=basePath%>imagenes/icons/accept.png' border='0' style='width: 20px;'>"
