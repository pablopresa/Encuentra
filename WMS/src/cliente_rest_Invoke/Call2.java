...
public class Call2 {
	...
	public static String loginWMS(String host)
	{
		HttpClient client = new DefaultHttpClient();
		HttpPost post = new HttpPost("http://"+host+":8889/token");
		post.setHeader("Content-Type", "application/x-www-form-urlencoded");
		...
		try {
		    ...
		    post.setEntity(new StringEntity("grant_type=password&username=WMS&password=WMS", "UTF-8"));
		    HttpResponse response = client.execute(post);
		    HttpEntity entity = response.getEntity();
		    String results = EntityUtils.toString(entity);
		    obj = new JSONObject(results);
	...
	{
		HttpClient client = new DefaultHttpClient();
		HttpPost post = new HttpPost("http://"+host+":8889/ArgentisApi/AddDocument?pDocuments="+pDocuments+"&pID=10");
		//HttpPost post = new HttpPost("http://200.58.149.126:8889/ArgentisApi/AddDocument?pDocuments=GoodsIssue");
		post.setHeader("Content-Type", "application/json");
		post.setHeader("Authorization", "Bearer "+token);
		String retorno = "";
